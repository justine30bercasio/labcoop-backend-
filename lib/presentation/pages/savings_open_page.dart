...
class _SavingsOpenPageState extends State<SavingsOpenPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _loading = false; _error = 'Server not responding. Tap Retry.'; });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _apply(String id, String name) async {
    ...
    if (ok != true) return;
    final r = await BankingApiService.applySavingsAccount(widget.accountId, id);
    if (!mounted) return;
    if (r != null) {
  ...
  Widget _productCard(Map<String, dynamic> product) {
    ...
    final minBal = (product['min_balance'] ?? 0).toDouble();
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(spacing: 16, children: [
              ...
              _infoChip(Icons.money_off, 'Min: PHP ${minBal.toStringAsFixed(0)}'),
