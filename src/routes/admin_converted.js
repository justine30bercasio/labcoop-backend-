...
router.get('/', requireSession, asyncHandler(async (req, res) => {
  ...
  ${(pendingLoans + pendingWithdrawals) > 0 ? `
  <div class="pending-alert">
    <span class="pa-icon">&#x26A0;</span>
    <span class="pa-text">${pendingLoans} pending loan${pendingLoans !== 1 ? 's' : ''}, ${pendingWithdrawals} withdrawal request${pendingWithdrawals !== 1 ? 's' : ''}</span>
...
router.get('/audit', requireSession, asyncHandler(async (req, res) => {
  ...
  var typeOpts = ['deposit','withdrawal','loan_disbursement','loan_payment','interest_credit','interest'];
  ...
  <div class="card">
    ...
    <div class="card-body">
      ${txns.length === 0 ? '<div style="text-align:center;padding:48px;color:var(--text-muted)">No transactions found for the selected filters.</div>' : '<table><tr><th>Receipt #</th><th>Date &amp; Time</th><th>Member</th><th>ID</th><th>Type</th><th>Amount</th><th>Balance Delta</th><th>Description</th><th>Ref</th></tr>' + txns.map(function(t) {
        var sign = (t.type==='deposit'||t.type==='loan_disbursement'||t.type==='interest_credit'||t.type==='interest') ? '+' : '-';
        var col = (t.type==='deposit'||t.type==='loan_disbursement'||t.type==='interest_credit'||t.type==='interest') ? '#16a34a' : '#dc2626';
