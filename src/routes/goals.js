...
router.post('/',
  ...
  (req, res) => {
    ...
    const goal = store.createGoal({ account_id, title, target_amount: Number(target_amount), category_icon: category_icon || 'savings' });
...
router.put('/:goalId',
  ...
  (req, res) => {
    ...
    const oldGoal = store.getGoal(req.params.goalId);
...
router.delete('/:goalId',
  ...
  (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
    store.deleteGoal(req.params.goalId);
    res.status(204).send();
  }
);
